...
// ============================================================
// PUBLIC/JS/ADMIN.JS  — browser-only, no require()
// ============================================================
...
let overrideApproveMode = false; // true = approve-result flow, false = force-winner flow
...
/**
 * api() – all admin API calls.
 * Routes are mounted at /admin/... on the server.
 */
...
async function loadWithdrawals() {
    ...
    try {
        // Server route: GET /admin/withdrawals?status=...
...
function renderWithdrawals(items) {
    ...
    items.forEach(w => {
        ...
        // ── SEND TO box — the most important info ──
        ...
        copyBtn.style.cssText = `
            ...
            color:#00cc44;border-radius:6px;padding:4px 10px;
...
qs('btn-submit-paid').addEventListener('click', async () => {
    ...
    try {
        const res = await api(`/withdrawals/${currentWithdrawalId}/approve`, {
            method: 'POST',
            body: JSON.stringify({ notes: `Paid. M-PESA: ${code}` })
        });
        const data = await res.json();
        if (!res.ok) return alert(data.error || 'Failed');
...
qs('btn-submit-reject').addEventListener('click', async () => {
    ...
    try {
        // Server route: POST /admin/withdrawals/:id/reject
...
// ============================================================
// TOURNAMENTS — fixed routes to match server tournaments.js
// Server routes are under /tournaments/admin/...
// But they are accessed via /admin/... prefix here — so we need
// the admin.js server route to proxy or the routes must align.
// Using /admin/tournaments/... paths (must match server admin.js)
// ============================================================
async function loadTournaments() {
    try {
        // Server route: GET /admin/tournaments
...
function renderTournaments(items) {
    ...
    items.forEach(t => {
        ...
        codeTd.textContent = t.name;
        const feeTd = document.createElement('td');
        feeTd.textContent = `KES ${t.entry_fee}`;
        const startTd = document.createElement('td');
        startTd.textContent = fmtDate(t.start_time);
        const playersTd = document.createElement('td');
        playersTd.textContent = `${t.current_players || 0}/${t.max_players}`;
        const roomTd = document.createElement('td');
        roomTd.textContent = t.room_code || '—';
...
qs('tournament-save').addEventListener('click', async () => {
    ...
    if (!body.name || !body.entry_fee || !body.start_time || !body.max_players) {
        return alert('Fill in all required fields.');
    }
    try {
        // Server routes: POST /admin/tournaments  or  PATCH /admin/tournaments/:id
...
qs('btn-confirm-delete').addEventListener('click', async () => {
    try {
        // Server route: DELETE /admin/tournaments/:id
...
async function loadFriendMatches() {
    ...
    try {
        // Server route: GET /admin/friend-matches?status=...
        const res  = await api(`/friend-matches?status=${status}`);
        const data = await res.json();
        renderFriendMatches(Array.isArray(data) ? data : []);
    } catch (err) {
        console.error('Friend matches load error:', err);
    } finally {
        qs('fm-loading').style.display = 'none';
    }
}
...
function renderFriendMatches(items) {
    ...
    items.forEach(m => {
        ...
        const codeTd = document.createElement('td');
        codeTd.style.cssText = 'font-family:monospace;color:#00ff41;';
        codeTd.textContent = code;

        const creatorTd = document.createElement('td');
        creatorTd.textContent = m.creator_username || (m.creator_id?.substring(0,8) || '—');

        const joinerTd = document.createElement('td');
        joinerTd.textContent = m.joiner_username || (m.joiner_id?.substring(0,8) || '—');

        const scoreTd = document.createElement('td');
        scoreTd.textContent = score;

        const wagerTd = document.createElement('td');
        wagerTd.textContent = `KES ${m.wager_amount}`;

        const prizeTd = document.createElement('td');
        prizeTd.textContent = `KES ${m.winner_prize}`;

        const statusTd = document.createElement('td');
        statusTd.innerHTML = statusBadge(m.status);

        const dateTd = document.createElement('td');
        dateTd.style.cssText = 'font-size:0.72rem;color:#555;';
        dateTd.textContent = fmtDate(m.created_at);

        const actTd = document.createElement('td');
        actTd.style.cssText = 'white-space:nowrap;';

        // Screenshot preview button — show if any screenshot exists
        const hasScreenshot = m.creator_screenshot_url || m.joiner_screenshot_url;
        if (hasScreenshot) {
            const ssBtn = document.createElement('button');
            ssBtn.className = 'btn btn-blue';
            ssBtn.textContent = '📸 View';
            ssBtn.style.marginRight = '4px';
            ssBtn.addEventListener('click', () => openScreenshotViewer(m));
            actTd.appendChild(ssBtn);
        }

        // Override button for any live/stuck match
        if (['active','pending_review','disputed','awaiting_confirmation','penalty_shootout'].includes(m.status)) {
            ...
            ovrBtn.className = 'btn btn-orange';
            ovrBtn.textContent = '⚡ Override';
            ovrBtn.style.marginRight = '4px';
        ...
        if (m.status === 'disputed') {
            ...
            dspBtn.className = 'btn btn-red';
            dspBtn.textContent = '⚖️ Resolve';
            dspBtn.addEventListener('click', () => openDisputeModal(m.id));
            actTd.appendChild(dspBtn);
        }

        // ============================================================
        // NEW: DELETE BUTTON (always available)
        // ============================================================
        const delBtn = document.createElement('button');
        delBtn.className = 'btn btn-red';
        ...
        delBtn.addEventListener('click', async () => {
            if (!confirm(`Delete match ${m.match_code || m.id}? This will remove all screenshots.`)) return;
            try {
                const res = await api(`/friend-matches/${m.id}`, { method: 'DELETE' });
                const data = await res.json();
                if (!res.ok) throw new Error(data.error || 'Delete failed');
                alert('Match deleted');
                loadFriendMatches(); // refresh list
            } catch (err) {
        ...
        tr.appendChild(joinerTd);
        tr.appendChild(scoreTd);
        tr.appendChild(wagerTd);
...
// ============================================================
// SCREENSHOT VIEWER — dedicated modal to view/compare shots
// ============================================================
function openScreenshotViewer(match) {
    lbCompareUrls = {
        left:  match.creator_screenshot_url || null,
        right: match.joiner_screenshot_url  || null,
    };

    // If only one screenshot, open single view; if both, open compare
    if (lbCompareUrls.left && lbCompareUrls.right) {
        ...
        const who = lbCompareUrls.left ? 'Creator Screenshot' : 'Joiner Screenshot';
        openLightbox(url, who);
    }
}

// ============================================================
// APPROVE RESULT — load match then open override modal in approve mode
// ============================================================
async function approveResult(matchId) {
    try {
        ...
        openOverrideModal(m, true /* approveMode */);
    } catch (err) { alert('Network error: ' + err.message); }
}
...
function openOverrideModal(match, approveMode = false) {
    console.log('[ADMIN] openOverrideModal called, match id:', match?.id, 'approveMode:', approveMode);
    currentOverrideMatch = match;
    overrideApproveMode  = approveMode;

    qs('ovr-code').textContent   = match.match_code || match.efootball_code || '—';
    ...
    // Reset selection state fully on every open
    ...
    qs('ovr-notes').value = '';
    // Adjust title + note placeholder for context
    ...
    if (notesEl) notesEl.placeholder = approveMode
        ...
        : 'e.g. Player A screenshot clearly shows final score 3–1. Confirmed via video.';

    ovrUpdateBtn();  // sync button state
...
function setOvrScreenshot(wrapId, linkId, tagId, url, label) {
    ...
    if (url) {
        ...
        none.className = 'ovr-ss-none';
        none.textContent = 'No screenshot uploaded';
...
function ovrUpdateBtn() {
    console.log('[ADMIN] ovrUpdateBtn — resolution:', ovrSelectedResolution, 'winner:', ovrSelectedWinnerId);
    ...
    if (hasSelection && notes) {
        ...
        btn.textContent = !hasSelection && !notes ? 'Select a winner and add notes'
            : !hasSelection ? 'Select a winner above'
            : 'Add admin notes below';
    }
}

// Wire buttons after DOM is ready — running at parse time means elements don't exist yet
document.addEventListener('DOMContentLoaded', () => {
    console.log('[ADMIN] DOMContentLoaded fired');
    console.log('[ADMIN] ovr-btn-creator el:', qs('ovr-btn-creator'));
    console.log('[ADMIN] ovr-submit-btn el:', qs('ovr-submit-btn'));
    qs('ovr-btn-creator').addEventListener('click', () => {
        console.log('[ADMIN] creator btn clicked, match:', currentOverrideMatch?.id);
    ...
    qs('ovr-btn-joiner').addEventListener('click', () => {
        console.log('[ADMIN] joiner btn clicked, match:', currentOverrideMatch?.id);
    ...
    qs('ovr-btn-draw').addEventListener('click', () => {
        console.log('[ADMIN] draw btn clicked');
        ovrSelectedWinnerId   = null;
    ...
    qs('ovr-submit-btn').addEventListener('click', async () => {
        console.log('[ADMIN] submit clicked — match:', currentOverrideMatch?.id, 'resolution:', ovrSelectedResolution, 'winner:', ovrSelectedWinnerId);
        ...
        try {
            const res  = await api(`/force-winner/${currentOverrideMatch.id}`, {
                method: 'POST',
                body:   JSON.stringify({
                    winnerId:   ovrSelectedWinnerId,
                    resolution: ovrSelectedResolution,
                    adminNotes: notes,
                }),
            });
            const data = await res.json();
            console.log('[Admin] override response:', res.status, data);
            if (!res.ok) {
            ...
            const prize = data.prizePaid ? ` KES ${data.prizePaid} paid.` : '';
            alert(`✅ Match settled.${prize}`);
            loadFriendMatches();
        } catch (err) {
...
async function openDisputeModal(matchId) {
    ...
    try {
        // Server route: GET /admin/friend-matches/:id
        const res = await api(`/friend-matches/${matchId}`);
        const m   = await res.json();
...
function populateDisputeModal(m) {
    qs('dsp-code').textContent   = m.match_code || m.efootball_code || '—';
    ...
    qs('dsp-cname').textContent = m.creator_username || '—';
    qs('dsp-cteam').textContent = m.creator_team     || '—';
    qs('dsp-jname').textContent = m.joiner_username  || '—';
    qs('dsp-jteam').textContent = m.joiner_team      || '—';

    qs('sc-cname').textContent  = m.creator_username || '—';
    qs('sc-cteam').textContent  = m.creator_team     || '—';
    qs('sc-jname').textContent  = m.joiner_username  || '—';
    qs('sc-jteam').textContent  = m.joiner_team      || '—';

    const cscore = m.declared_score_creator;
    const jscore = m.declared_score_joiner;
    ...
    // Screenshots — use dedicated setter
    ...
    const events = [
        { time: m.created_at,   label: 'Match created' },
        { time: m.started_at,   label: 'Match started' },
        { time: m.declared_at,  label: 'Score declared' },
        { time: m.disputed_at,  label: '⚠️ Disputed' },
    ].filter(e => e.time);
    ...
    events.forEach(ev => {
        ...
        timeEl.textContent = fmtDate(ev.time);
...
function setDspImg(wrapId, linkId, url, label) {
    ...
    if (url) {
        ...
        const none = document.createElement('div');
        none.className = 'dp-noimg';
        none.textContent = 'No screenshot provided';
        wrap.appendChild(none);
        link.style.display = 'none';
    }
}
...
async function resolveDispute(winnerId, resolution) {
    ...
    const label = resolution === 'draw'
        ? 'refund both players (draw)'
    ...
    try {
        // Server route: POST /admin/resolve-dispute/:id
        const res  = await api(`/resolve-dispute/${currentDisputeMatch.id}`, {
            method: 'POST',
            body: JSON.stringify({ winnerId, resolution, adminNotes: notes })
        });
        const data = await res.json();
        if (!res.ok) {
...
async function loadAnalytics() {
    ...
    try {
        // Server route: GET /admin/analytics
        const res  = await api('/analytics');
        const data = await res.json();
        if (!res.ok) {
            container.innerHTML = `<div style="color:#ff4444;padding:20px;">${escapeHtml(data.error || 'Failed to load analytics')}</div>`;
            return;
        }
        renderAnalytics(data);
    } catch (err) {
        container.innerHTML = `<div style="color:#ff4444;padding:20px;">Network error: ${escapeHtml(err.message)}</div>`;
    }
}
...
function statCard(label, value, sub) {
    ...
    labelEl.style.cssText = 'font-size:0.65rem;letter-spacing:2px;text-transform:uppercase;color:#555;margin-bottom:6px;';
    labelEl.textContent = label;

    const valEl = document.createElement('div');
    valEl.style.cssText = 'font-size:1.8rem;font-weight:800;color:#f0f0f0;line-height:1;';
    ...
    if (sub) {
        ...
        subEl.style.cssText = 'font-size:0.72rem;color:#555;margin-top:5px;';
...
function renderAnalytics(d) {
    ...
    const sections = [
        ...
        { title: '⚽ Matches', cards: [
            ...
            ['MTD COMPLETED',    m.mtdCompleted   || 0],
        ...
        { title: '👥 Users', cards: [
            ...
            ['NEW TODAY',        u.newToday   || 0],
            ['NEW MTD',          u.newMtd     || 0],
            ['ACTIVE 7 DAYS',    u.active7Days|| 0],
        ]},
    ...
    sections.forEach(sec => {
        ...
        titleEl.style.cssText = 'font-size:1.1rem;font-weight:700;margin:24px 0 12px;color:#888;';
    ...
    if (d.dailyChart?.length) {
        ...
        chartTitle.style.cssText = 'font-size:1.1rem;font-weight:700;margin:24px 0 12px;color:#888;';
        chartTitle.textContent = '📈 Last 30 Days — Daily Volume';
        container.appendChild(chartTitle);

        const maxVol = Math.max(...d.dailyChart.map(r => r.volume), 1);
        const chartEl = document.createElement('div');
        chartEl.style.cssText = 'display:flex;align-items:flex-end;gap:3px;height:80px;background:#0d0d10;border:1px solid #1e1e26;border-radius:14px;padding:10px 12px;overflow:hidden;';
        d.dailyChart.forEach(row => {
            const bar = document.createElement('div');
            const pct = Math.round((row.volume / maxVol) * 100);
            bar.style.cssText = `flex:1;background:rgba(0,255,65,0.3);border-radius:2px 2px 0 0;height:${Math.max(pct, row.volume > 0 ? 4 : 0)}%;transition:height 0.3s;`;
...
// ============================================================
// LIGHTBOX — zoom, pan, side-by-side compare
// ============================================================
function openLightbox(url, title) {
    lbScale = 1; lbOffX = 0; lbOffY = 0;
    lbMode = 'single';
    ...
    // Use class toggle instead of style.display for transition support
...
function openCompare(leftUrl, rightUrl, leftLabel, rightLabel) {
    ...
    function setPane(wrapId, url) {
        ...
        if (url) {
            ...
            const none = document.createElement('div');
            none.className = 'lb-no-img';
            none.textContent = 'No screenshot';
            wrap.appendChild(none);
        }
    }
    setPane('lb-cmp-left-wrap',  leftUrl);
    setPane('lb-cmp-right-wrap', rightUrl);
...
function applyLbTransform() {
    ...
    img.style.transform = `translate(${lbOffX}px,${lbOffY}px) scale(${lbScale})`;
    qs('lb-zoom-label').textContent = Math.round(lbScale * 100) + '%';
}

function closeLightbox() {
    qs('lightbox').classList.remove('open');
    qs('lb-compare').classList.remove('active');
}
...
lbSingle.addEventListener('mousedown', e => {
    ...
    lbDragging = true; lbDragX = e.clientX - lbOffX; lbDragY = e.clientY - lbOffY;
    lbSingle.classList.add('grabbing');
});
window.addEventListener('mousemove', e => {
    if (!lbDragging) return;
    lbOffX = e.clientX - lbDragX; lbOffY = e.clientY - lbDragY;
    applyLbTransform();
});
